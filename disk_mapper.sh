...
for d in $(ls -1 /dev/ | grep -P "sd\S$"); 
    do full_output=$(smartctl -a /dev/$d);
        ...
        serial=$(echo "$filtered_output" | grep Serial | cut -d ':' -f 2-99 | sed -E "s/^\s+//g"); 
        capacity=$(echo "$filtered_output" | grep Capaci | cut -d ':' -f 2-99 | sed -E "s/^\s+//g"); 
        model=$(echo "$filtered_output" | grep Model | cut -d ':' -f 2-99 | sed -E "s/^\s+//g");
        version=$(echo "$filtered_output" | grep Version | cut -d ':' -f 2-99 | sed -E "s/^\s+//g");
